...
export async function getWordFromDictionary(search: string): Promise<IWord | undefined> {
	try {
		...
		return filteredWords.reduce((previousValue, currentValue, currentIndex): IWord => {
		...
		console.error(`There as a problem looking for the word ${search} in the dictionary: ${e}`);
		return undefined;
	}
	 ...
	 * @param raeResponse
	 */
	async function transformFromRAEtoIWord(raeResponse: SearchWordResponse): Promise<IWord[]> {
		return Promise.all(
