...
const PointsComponent: FunctionComponent = () => {
  ...
  const currentWordPoints = currentWordExists
    ? sumPointsFromCells(selectedCells)
    : 0;

  return (
    <div className="points">
      <div className="points__total">
        {totalPoints || currentWordExists ? <div>{totalPoints}</div> : null}
      </div>
      <div className="points__plus">{currentWordPoints ? "+" : null}</div>
      <div className="points__current">
        {currentWordPoints ? <div> {currentWordPoints}</div> : null}
      </div>
    </div>
  );
};
